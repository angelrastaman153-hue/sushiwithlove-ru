...
### js/data.js — **ЗДЕСЬ МЕНЯЙ МЕНЮ И ЦЕНЫ**
- `CONFIG` — телефон, адрес самовывоза, пороги доставки
- `DELIVERY_ZONES` — зоны и стоимость доставки
- `PROMO_CODES` — промокоды (добавить новый = 1 строка)
- `CATEGORIES` — список категорий с иконками
- `MENU_ITEMS` — все позиции меню
...
## Как добавить новый товар в меню

Открой `js/data.js`, добавь объект в `MENU_ITEMS`:

```js
{
  id: 99,               // Уникальный ID (любое число)
  category: 'rolls',    // ID категории из CATEGORIES
  name: 'Название',
  desc: 'Состав блюда',
  weight: '280 г · 8 шт',
  price: 490,           // Цена в рублях
  badge: 'new',         // 'hit' | 'new' | 'sale' | null
  img: '../photos/rolls/my-photo.jpg',  // путь к фото или null
  upsell: [101, 201],   // ID товаров для блока "Часто берут вместе"
},
```
